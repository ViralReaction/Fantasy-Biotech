...
{
    public class WorkGiver_DoGestate_Construct : WorkGiver_Scanner
    {
        ...
        private Job StartOrResumeBillJob(Pawn pawn, IBillGiver giver, bool forced = false)
        {
            ...
            for (int i = 0; i < giver.BillStack.Count; i++)
            {
                ...
                // Bill_ProductionWithUft specific handling
                ...
                // Bill_Autonomous specific handling
                if (bill is Bill_Autonomous bill_Autonomous && bill_Autonomous.State != FormingState.Gathering)
                {
                    return WorkOnFormedBill((Thing)giver, bill_Autonomous);
                }

                // Ingredient finding logic
                ...
                if (bill is Bill_Medical bill_Medical2 && bill_Medical2.uniqueRequiredIngredients?.Any() == true)
                {
                    foreach (Thing ingredient in bill_Medical2.uniqueRequiredIngredients)
                    {
                        if (ingredient.IsForbidden(pawn) || !pawn.CanReserveAndReach(ingredient, PathEndMode.OnCell, Danger.Deadly))
                        {
                            tmpMissingUniqueIngredients.Add(ingredient);
                        }
                    }
                }

                if (!TryFindBestBillIngredients(bill, pawn, (Thing)giver, chosenIngThings, list) || tmpMissingUniqueIngredients.Any())
                {
                    continue;
                }
        ...
        private static bool TryFindBestIngredientsHelper(Predicate<Thing> thingValidator, Predicate<List<Thing>> foundAllIngredientsAndChoose, List<IngredientCount> ingredients, Pawn pawn, Thing billGiver, List<ThingCount> chosen, float searchRadius)
        {
            ...
            int adjacentRegionsAvailable = 0;
            foreach (Region region in rootReg.Neighbors)
            {
                if (entryCondition(rootReg, region))
                {
                    adjacentRegionsAvailable++;
                }
            }
